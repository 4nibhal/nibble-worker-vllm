...
class vLLMEngine:
    def __init__(self, engine=None):
        ...
        self.max_concurrency = self._read_int_env(
            "MAX_CONCURRENCY", DEFAULT_MAX_CONCURRENCY
        )
        self.default_batch_size = self._read_int_env(
            "DEFAULT_BATCH_SIZE", DEFAULT_BATCH_SIZE
        )
        self.batch_size_growth_factor = self._read_int_env(
            "BATCH_SIZE_GROWTH_FACTOR",
            DEFAULT_BATCH_SIZE_GROWTH_FACTOR,
            aliases=("DEFAULT_BATCH_SIZE_GROWTH_FACTOR",),
        )
        self.min_batch_size = self._read_int_env(
            "MIN_BATCH_SIZE",
            DEFAULT_MIN_BATCH_SIZE,
    ...
    def _read_bool_env(self, key: str, default: bool):
        ...
        if value in ("false", "0", "no", "off"):
            return False
        logging.warning("Invalid %s=%r; using default %s", key, raw_value, default)
        return default
    ...
    async def _generate_vllm(
        ...
        last_output_texts, token_counters = (
